<?php
    ...
    function addComentario($Id_Evento,$Id_Comentario,$nick,$mail,$fecha,$hora,$texto){
        ...
        $mysqli_bd->real_escape_string($hora);
        $mysqli_bd->real_escape_string($texto);
    ...
    function getComentarios($Id_Evento){
        ...
        purgeId($Id_Evento) ;
        $resultado = $mysqli_bd->query("Select Id_Comentario, Nombre, Fecha, Hora, Comentario From Comentarios Where Id_Evento=" . $Id_Evento) ;

        $comentario = array('nombre' => "Error", 'comentario' => '404') ;
        $comentarios = array() ;

        while($fila = $resultado->fetch_assoc()){
            $comentario = array('id_com'=>$fila['Id_Comentario'],'nombre' => $fila['Nombre'], 'fecha' => date('d-m-Y',strtotime($fila['Fecha'])), 'hora' => $fila['Hora'],
    ...
    function modifyComentario($idEv,$idComm,$contenido){
        ...
        $mysqli_bd->real_escape_string($contenido);
